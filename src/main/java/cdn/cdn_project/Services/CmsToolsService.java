...
public class CmsToolsService {
    ...
    @Tool(name = "create_content", description = "Use this tool when the user wants to create one content (movie/series).")
    ...
    @Tool(name = "delete_content", description = "Deletes a content (movie/series) by its id.")
    public Map<String,Object> deleteContent(@ToolParam(description = "the content id") String id) {
        contentPostgresLocalServer.deleteContent(id);
        return Map.of("status","deleted","id",id);
    }

    @Tool(name = "update_content", description = "Updates a content (movie/series). Requires the content id.")
    public Map<String,Object> updateContent(@ToolParam(description = "the content id") String id,
    ...
    @Tool(name = "batch_create_contents", description = "Batch-creates multiple contents (movies/series) when the user wants to create more than one at once.")
    ...
    @Tool(name = "search_contents", description = "When user asks for info about contents or wants to perform an operation without specifying the content's id use this tool. casing doesnt matter on the query whenever you find any data respond, and if you hadn't found after trying both content type of movie and series respond to the user saying library doesnt have the content you are looking for")
    public Map<String,Object> searchContents( String query,
                                             @ToolParam(description = "movie or series") ContentType contentType) {
    ...
    @Tool(name = "batch_delete_contents", description = "Deletes multiple contents at once by their ids.")
    public Map<String,Object> batchDeleteContents(@ToolParam(description = "ids to delete") List<String> ids) {
        contentPostgresLocalServer.deleteContents(ids);
        return Map.of("status","deleted","ids",ids);
    }

    @Tool(name = "create_cast", description = "Creates a cast (actor/director) by their name and type. Optionally attach it to existing content ids.")
    ...
    @Tool(name = "update_cast", description = "Updates a cast (actor/director) by its id.")
    public Map<String,Object> updateCast(@ToolParam(description = "the cast id") Integer id,
    ...
    @Tool(name = "search_casts", description = "Searches for actors or directors by name when the user's details are incomplete. Use this to find cast ids or details required for other operations, or to check whether a cast exists.")
    public Map<String,Object> searchCasts(String query,
                                          @ToolParam(description = "actor or director") CastType castType) {
        Pageable limit = PageRequest.of(0, 5);
    ...
    @Tool(name = "delete_cast", description = "Deletes a cast (actor/director) by its id.")
    public Map<String,Object> deleteCast(@ToolParam(description = "id to delete") Integer id) {
        castPostgresLocalService.deleteCast(id);
        return Map.of("status","deleted","id",id);
    }

    @Tool(name = "batch_delete_casts", description = "Deletes multiple casts at once by their ids.")
    public Map<String,Object> batchDeleteCasts(@ToolParam(description = "ids to delete") List<Integer> ids) {
